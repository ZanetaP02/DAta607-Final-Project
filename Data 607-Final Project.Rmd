...
names(df_pop) <- c("State", "Y18", "Y20", "Y22")
df_pop
```
...
```{r}
dp <- df_pop %>%
  pivot_longer(cols = c('Y18', 'Y20', 'Y22'), names_to = "Years", values_to = "Drivers_Population")
dp
```
...
```{r}
ggplot(data = dp, aes(x = State, y = Drivers_Population, fill = Years)) + 
  geom_col(position = position_dodge()) + 
...
```{r}
df <- df_pop %>%
  pivot_longer(cols = c('Y22', 'Y20', 'Y18'), names_to = "Years", values_to = "Drivers_Population") %>%
...
ggplot(data = df, aes(x = State, y = Drivers_Population, fill = Years)) + 
  geom_col(position = position_dodge()) + 
  ggtitle("Graph of Drivers Population") +
  theme(axis.text.x = element_text(angle = 66, hjust = 1)) +
  labs(y = "Drivers Population")
...
```{r}
year_pop <- read.csv("https://raw.githubusercontent.com/ZanetaP02/DAta607-Final-Project/refs/heads/main/year_pop.csv")
head(year_pop)
```
...
dp <- df_pop %>%
  pivot_longer(cols = c('Y22', 'Y20', 'Y18'), names_to = "Years", values_to = "Drivers_Population") %>%
  mutate(Total_Drivers_Per_Years = recode(Years,
                                          "Y22" = "2022",
                                          "Y20" = "2020",
                                          "Y18" = "2018"),
         Drivers_Population = as.numeric(gsub("[^0-9]", "", Drivers_Population)))
