...
```{r}
fd1 <- Fatalitydata %>% as.data.frame() %>% rownames_to_column("state")
fd1$State <- tolower(fd1$State)
```

```{r}
ggplot(fd1, aes(map_id = State)) + geom_map(aes(fill = Y22), map = states) +
  expand_limits(x = states$long, y = states$lat) +
  scale_fill_gradient(low = "white", high = "blue") +
  labs(title = "Fatal Accidents Per State, 2022", x = "longitude", y = "latitude") +
  coord_fixed(1.3) +
  theme(panel.background = element_blank())
ggplot(fd1, aes(map_id = State)) + geom_map(aes(fill = Y20), map = states) +
  expand_limits(x = states$long, y = states$lat) +
  scale_fill_gradient(low = "white", high = "green") +
  labs(title = "Fatal Accidents Per State, 2020", x = "longitude", y = "latitude") +
  coord_fixed(1.3) +
  theme(panel.background = element_blank())
ggplot(fd1, aes(map_id = State)) + geom_map(aes(fill = Y18), map = states) +
  expand_limits(x = states$long, y = states$lat) +
...
### Top 3 High Fatalitities States
```{r}
dpop1 %>% top_n(3)
```
### Top 3 Low Fatalitities States
```{r}
dpop1 %>% top_n(-3)
```

The graph effectively visualizes traffic fatalities across US states for 2018, 2020, and 2022, highlighting significant variations between populous states like California, Florida, and Texas compared to smaller states.

Key Observations:
 - High Fatalities States: California, Florida, and Texas consistently have the highest fatalities rates.
 - Low Fatalities States: District of Columbia, Rhode Island, and Vermont have the lowest rates.
